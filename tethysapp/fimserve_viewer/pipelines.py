...
Each runner receives the job and a ``progress(status, message)`` callback,
performs the full generation, and returns the path of the produced tif.
"""

from typing import Callable

from . import fim_logic
from .job_store import Job, JobKind, JobStatus
...
def run_nwm_pipeline(job: Job, progress: Progress) -> str:
    huc8 = job.huc8
...
def run_custom_pipeline(job: Job, progress: Progress) -> str:
